...
void ChildWrapper::Init(Handle<Object> exports)
{
    ...
    tpl->PrototypeTemplate()->Set(v8::String::NewSymbol("getNum"),
        v8::FunctionTemplate::New(getNum)->GetFunction());";
    tpl->PrototypeTemplate()->Set(v8::String::NewSymbol("setNum"),
        v8::FunctionTemplate::New(setNum)->GetFunction());";
...
Handle<Value> ChildWrapper::int getNum();(const Arguments& args)
{
    HandleScope scope;
    ChildWrapper* obj = ObjectWrap::Unwrap<ChildWrapper>(args.This());
    return scope.Close(Number::New(obj->CHILD->getNum();
}
Handle<Value> ChildWrapper::void setNum(int num);(const Arguments& args)
{
    HandleScope scope;
    ChildWrapper* obj = ObjectWrap::Unwrap<ChildWrapper>(args.This());
    return scope.Close(